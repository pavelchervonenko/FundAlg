...
int main()
{
    ...
    int convex = is_convex(4, 0.0, 0.0, 2.0, 1.0, 1.0, 4.0, 0.0, 0.0); // Невыпуклый
    printf("Convex: %d\n", convex);

    convex = is_convex(3, 0.0, 0.0, 1.0, 1.0, 2.0, 0.0); // Выпуклый
    printf("Convex: %d\n", convex);

    // Пример использования polynomial_value
    double value = polynomial_value(2.0, 2, 1.0, 2.0, 3.0); // x^2 + 2x + 3 при x = 2
    printf("Polynomial value: %f\n", value);

    // Пример использования kaprekar_numbers
    kaprekar_numbers(10, "1", "45", "9", "297", NULL);
    kaprekar_numbers(17, "A", "10", NULL); // Система счисления 17
